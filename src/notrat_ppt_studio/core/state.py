...
def deck_dir_from_target(target: str) -> Path:
    ...
    if path.is_dir():
        return path
    if path.name == "deck.manifest.json":
        return path.parent
    raise ValueError(f"Expected deck directory or deck.manifest.json: {target}")


def deck_manifest_path(deck_dir: Path) -> Path:
    return Path(deck_dir) / "deck.manifest.json"


def run_state_path(deck_dir: Path) -> Path:
    return Path(deck_dir) / "state/run.json"


def load_jobs(deck_dir: Path) -> Dict[str, Any]:
    return read_json(deck_manifest_path(deck_dir))


def save_jobs(deck_dir: Path, jobs: Dict[str, Any]) -> None:
    write_json(deck_manifest_path(deck_dir), jobs)


def locked_jobs(deck_dir: Path):
    return locked_json(deck_manifest_path(deck_dir))
...
def find_slide(jobs: Dict[str, Any], slide: Any) -> Dict[str, Any]:
    ...
    for entry in jobs.get("slides", []):
        if entry.get("slide_id") == slide_id or str(entry.get("number")) == str(slide):
            return entry
    raise KeyError(f"Slide not found in deck.manifest.json: {slide_id}")
