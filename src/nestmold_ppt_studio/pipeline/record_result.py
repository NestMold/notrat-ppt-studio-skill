#!/usr/bin/env python3
"""Record and copy a nestmold-ppt-studio slide image generated by a subagent."""
...
def _validate_backend(backend: str) -> str:
    ...
    if "image" not in lowered and "built-in" not in lowered:
        raise SystemExit(
            ...
            "`built-in image tool` or `nestmold-ppt image`."
        )
    return value
...
def _backend_family(value: str) -> str:
    lowered = value.lower()
    if "nestmold-ppt image" in lowered:
...
def main() -> int:
    ...
    with locked_jobs(deck_dir) as jobs:
        ...
        out_ref = slide.get("out") or f"assets/slides/{slide['slide_id']}.png"
