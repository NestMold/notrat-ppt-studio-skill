...
def get_slide_images(ppt_project_dir: str) -> List[str]:
    ...
    Args:
        ppt_project_dir: PPT 项目目录（包含 assets/slides 子目录）

    Returns:
        按顺序排列的图片文件路径列表
    """
    # 从 assets/slides 子目录读取
    slide_assets_dir = os.path.join(ppt_project_dir, "assets/slides")

    if not os.path.exists(slide_assets_dir):
        print(f"错误：assets/slides 目录不存在: {slide_assets_dir}")
        return []

    print(f"从 assets/slides 目录读取图片: {slide_assets_dir}")
    ...
    image_files = []
    for file in os.listdir(slide_assets_dir):
        file_path = os.path.join(slide_assets_dir, file)
...
def load_speaker_notes(ppt_project_dir: str) -> Dict[int, str]:
    """
    从 content/speaker-notes.md 读取每页演讲备注。
    ...
    - ## 第 1 页：标题
    """
    speech_path = os.path.join(ppt_project_dir, "content/speaker-notes.md")
...
文件夹结构要求:
  ...
  │   ├── slide_02.png
  │   └── ...
  ├── content/speaker-notes.md (可选，将写入 PPT 备注)
  └── MyPresentation.pptx (将在此生成)

注意：
  - 图片文件必须放在 assets/slides 子目录中
  - 只会读取 slide_01.png、slide_02.png 这类正式图片，其他图片会被忽略
  - 图片文件按页码排序
  - 建议图片文件命名为: slide_01.png, slide_02.png, ...
  - 每张图片会充满整个幻灯片页面
  - 如果项目目录下存在 content/speaker-notes.md，会按 Slide N 标题写入每页备注
        '''
    )
    ...
    parser.add_argument('--init',
                        action='store_true',
                        help='只创建 PPT 项目目录和 assets/slides 子目录，不生成 PPT')
    ...
    if args.init:
        ...
        print(f"✓ 幻灯片图片目录已准备好: {slide_assets_dir}")
        sys.exit(0)
    ...
    if not image_files:
        ...
        print(f"\n请将幻灯片图片放置在: {slide_assets_dir}/")
        sys.exit(1)

    print(f"找到 {len(image_files)} 张幻灯片图片\n")
    speaker_notes = load_speaker_notes(ppt_project_dir)
    if speaker_notes:
        print(f"找到 {len(speaker_notes)} 页备注: {os.path.join(ppt_project_dir, 'content/speaker-notes.md')}\n")
